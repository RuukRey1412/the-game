...
const CARDS = {
    ...
    def: [
        ...
        { name: "TOIEC800点", def: 8, mp: 8, weight: 6, sex: "無", desc: "MP10回復 & 手札+2", effect: async (u) => { u.mp = Math.min(MAX_MP, u.mp + 10); if(u.id === myRole) { drawCard(u); drawCard(u); } return "MP10回復&手札+2"; } },
    ...
    sup: [
        ...
        { name: "てつやのリーダーシップ", mp: 8, weight: 15, desc: "HP10回復, MP10回復", effect: async (u) => { u.hp = Math.min(MAX_HP, u.hp + 10); u.mp = Math.min(MAX_MP, u.mp + 10); return "HP10&MP10回復"; } },
        { name: "和成のリーダーシップ", mp: 7, weight: 15, desc: "HP10回復", effect: async (u) => { u.hp = Math.min(MAX_HP, u.hp + 10); return "HP10回復"; } },
        { name: "そうすけの威厳", mp: 30, weight: 5, desc: "相手に20ダメージ", effect: async (u, t) => { t.hp -= 20; return "相手に20ダメージ！"; } },
        { name: "ゆうすけの尊厳", mp: 20, weight: 10, desc: "手札2枚得る", effect: async (u) => { if(u.id === myRole) { drawCard(u); drawCard(u); } return "手札+2"; } },
        { name: "りりこさんの知見", mp: 25, weight: 10, desc: "手札3枚得る", effect: async (u) => { if(u.id === myRole) { drawCard(u); drawCard(u); drawCard(u); } return "手札+3"; } },
        ...
        { name: "みっちーの簿記", mp: 10, weight: 10, desc: "手札2枚得る", effect: async (u) => { if(u.id === myRole) { drawCard(u); drawCard(u); } return "手札+2"; } },
        { name: "かいせの発音", mp: 20, weight: 10, desc: "手札3枚得る", effect: async (u) => { if(u.id === myRole) { drawCard(u); drawCard(u); drawCard(u); } return "手札+3"; } }
    ]
};
...
socket.on('sync-action', async (data) => {
    ...
    if (data.type === 'use') await executeCard(actor, data.idx);
        ...
        if(phase === "DRAW") {
            phase = "MAIN";
            log(`${actor.id.toUpperCase()}がカードを引いた`);
        }
    }
    isProcessing = false; 
    updateUI();
...
function renderHand(id, p) {
    ...
    p.hand.forEach((c, i) => {
        ...
        d.onmouseover = () => { 
            ...
            if(detail) detail.innerText = `${c.name}: ${c.desc}`; 
        };
        el.appendChild(d);
    });
}
...
// ダメージ計算の統合
function getDamage(attackerCard, defenderCard) {
    let atkPower = attackerCard.calcAtk ? attackerCard.calcAtk(defenderCard) : attackerCard.atk;
    let defPower = defenderCard ? (defenderCard.calcDef ? defenderCard.calcDef(attackerCard) : (defenderCard.def || 0)) : 0;
    return Math.max(0, atkPower - defPower);
}
...
function executeSkip(p) {
    if (phase === "DEFENSE") {
        let dmg = getDamage(currentAttack, null); // 防御カードなし
        p.hp -= dmg;
...
function checkWin() {
    const p1Lost = p1.hp <= 0 || (p1.hand.length === 0 && turn === p1 && phase !== "DRAW");
    const p2Lost = p2.hp <= 0 || (p2.hand.length === 0 && turn === p2 && phase !== "DRAW");
    
    if (p1Lost || p2Lost) {
        isProcessing = true; // 操作ロック
        const overlay = document.getElementById('overlay');
        const msg = document.getElementById('winner-msg');
        if(overlay) overlay.style.display = "flex";
        if(msg) msg.innerText = (p1Lost ? "PLAYER B" : "PLAYER A") + " WIN!";
        return true;
    }
    return false;
}

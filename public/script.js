...
const CARDS = {
    ...
    sup: [
        ...
        { name: "りりこさんの知見", mp: 25, weight: 9.8, desc: "手札3枚得る", effect: async (u) => { if(u.id === myRole) { drawCard(u); drawCard(u); drawCard(u); } return "手札+3"; } },
        ...
        { name: "かいせの発音", mp: 20, weight: 9.8, desc: "手札3枚得る", effect: async (u) => { if(u.id === myRole) { drawCard(u); drawCard(u); drawCard(u); } return "手札+3"; } },
        { name: "盛川美優の妄想", mp: 40, weight: 2.0, desc: "※利用可能な場合、必ず利用しなければならない。使用時、敗北する。", effect: async (u) => { u.hp = 0; return "強制敗北！"; } }
    ]
};
